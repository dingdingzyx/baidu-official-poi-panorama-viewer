...
- 一次查询只接受一个城市和一个关键词。用户点击“下一页”才请求下一页，最多手动查看官方边界内的 20 页/400 条；不后台预取、不自动换词、不自动遍历城市。
...
2. 在控制台开通地点检索对应服务，选择 **IP 校验**并创建/获取 Server AK；本工具不实现 SN 签名模式。
...
常见 `210` 表示当前公网出口 IP 没有匹配 Server AK 白名单，`211` 表示应用使用了本工具不支持的 SN 校验方式。
...
常见 `220` 表示 Browser AK 的 Referer 白名单未匹配页面来源。地点检索的官方状态码中，`4`/`302` 是配额问题，`5`/`200` 是 AK 或应用无效，`203` 是应用类型错误，`210`/`211` 是 Server AK 校验失败，`240` 是服务未开通，`401` 是官方并发限制；界面会给出对应处理方向。
...
macOS/Linux 将第二行改为 `cp .env.example .env`，其余命令相同。
