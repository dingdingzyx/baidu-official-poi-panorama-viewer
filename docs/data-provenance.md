...
| --- | --- | --- | --- |
| POI page | Documented Baidu Place API | Immediate local display and back navigation | Current browser page memory only; cleared on refresh |
...
The application is an interface to the account holder's authorized official services. Its 20-page/400-result boundary follows the documented search limit but does not mean a city or keyword is complete. It does not assert ownership of source data or guarantee that a response is complete, current, or suitable for any use beyond the applicable official terms.
